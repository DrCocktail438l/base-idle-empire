...
function App() {
  ...
  const claimResources = () => {
    const production = Math.floor((mineLevel * 62) + (farmLevel * 52) + (labLevel * 68) + (towerLevel * 105) + (vaultLevel * 82))
  ...
  const upgradeMine = () => {
    const cost = Math.floor(230 + (mineLevel * 115))
    if (resources >= cost) {
  ...
  const upgradeFarm = () => {
    const cost = Math.floor(280 + (farmLevel * 125))
    if (resources >= cost) {
      setResources(resources - cost)
  ...
  const upgradeLab = () => {
    const cost = Math.floor(520 + (labLevel * 130))
    if (resources >= cost) {
      setResources(resources - cost)
  ...
  const upgradeTower = () => {
    const cost = Math.floor(780 + (towerLevel * 210))
    if (resources >= cost) {
      setResources(resources - cost)
  ...
  const upgradeVault = () => {
    const cost = Math.floor(1100 + (vaultLevel * 270))
    if (resources >= cost) {
      setResources(resources - cost)
  ...
  const prestigeReset = () => {
    if (resources < 45000) {
      alert("You need at least 45,000 resources to prestige!")
      return
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center text-xs text-gray-500 mt-16">
          Commit 35/100 • The empire continues to grow!
        </div>
      </div>
    </div>
  )
